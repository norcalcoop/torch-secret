...
Plans:
- [x] 40-01-PLAN.md — Argon2id hardening: createVerifyTightLimiter (5/min), p-limit concurrency cap, payload limit 100KB (Zod + error-handler 413), E2E gate hardening, passOnStoreError warn logging
- [x] 40-02-PLAN.md — PostgreSQL pool hardening: max 10, idleTimeoutMillis, connectionTimeoutMillis, statement_timeout, pool error logging
- [x] 40-03-PLAN.md — Wave 0 test scaffolds: create notification.service.test.ts (ZK invariant tests green), auth.test.ts (stubs), webhooks.test.ts (stubs)
- [x] 40-04-PLAN.md — All 7 integration/unit tests: IDOR dashboard, session logout, Pro-gate re-validation, Stripe signature, race condition, expiration worker soft/hard delete
- [x] 40-05-PLAN.md — CONCERNS.md quick wins: console.error → logger.error in notification.service.ts + subscribers.service.ts; OAuth account-linking audit comment in auth.ts
